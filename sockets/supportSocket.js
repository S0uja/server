...
const sendMessageToAdmins = (message, room, target) => {
	admins.forEach(admin => {
		if (admin.room === room) {
			admin.ws.send(jsonMessage(message, room, target, 'user'))
		}
	})
}
...
const messageUserHandler = (ws, req) => {
	...
	users[connectionId].on('message', msg => {
		...
		pushTempStorage(msg.message, msg.room, msg.target, 'user')
...
const messageAdminHandler = (ws, req) => {
	...
	ws.on('message', msg => {
		...
		users[msg.room].send(
			jsonMessage(msg.message, msg.room, msg.target, 'server')
		)

		pushTempStorage(msg.message, msg.room, msg.target, 'server')
	})
}
